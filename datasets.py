        ...
        # Mask background using segmentation mask and crop to bounding box.
            ...
            if (not np.any(pd.isnull(segmentation))):
                ...
                if isinstance(segmentation, list):
                    # Explicitly convert polygon list to a single compressed RLE object
                    h, w = img.size
                    # The `segmentation` variable is a list of polygons, so we pass it to `frPyObjects`
                    rles = mask_coco.frPyObjects(segmentation, h, w)
                    segmentation = mask_coco.merge(rles) # Merge multiple polygons if necessary
                #END OF EDIT    
                mask = mask_coco.decode(segmentation).astype("bool")
                img = Image.fromarray(img * mask[..., np.newaxis])
